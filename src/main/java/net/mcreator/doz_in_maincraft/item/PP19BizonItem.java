...
public class PP19BizonItem extends Item {
	public PP19BizonItem() {
		super(new Item.Properties().durability(6000).rarity(Rarity.COMMON));
	}
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player entity, InteractionHand hand) {
		...
		if (PP19BizonMozhnoIspolzovatDalnoboinyiPriedmietProcedure.execute(ar.getObject()))
	...
	public void onUseTick(Level world, LivingEntity entity, ItemStack itemstack, int count) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				PP19BizonProjectileEntity projectile = PP19BizonProjectileEntity.shoot(world, entity, world.getRandom());
	...
	private ItemStack findAmmo(Player player) {
		return new ItemStack(PP19BizonProjectileEntity.PROJECTILE_ITEM.getItem());
	}
}
